...
import { dict } from "@/content/familien-i18n";
...
export default async function FamilienPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: raw } = await params;
  const locale: Locale = isLocale(raw) ? raw : defaultLocale;
  const t = dict[locale];
    ...
    <>
      ...
      <section className="bg-cream-deep/60">
        <Container className="py-16">
          ...
          <p className="mt-6 max-w-2xl text-sm text-ink-soft">{t.supportText}</p>
        </Container>
      </section>
      ...
      <CTASection title={t.ctaTitle} primaryLabel={t.ctaButton} primaryHref={href("/hilfe-starten?anliegen=papierkram")} />
    </>
  );
}
