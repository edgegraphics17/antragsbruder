    ...
    <ol className="relative">
      ...
      {phases.map((p) => (
        <li key={p.phase} className="relative mb-10 last:mb-0 sm:pl-16">
          <span className="absolute left-0 top-1 hidden h-10 w-10 items-center justify-center rounded-full border-4 border-cream bg-green-900 text-xs font-bold text-cream sm:flex">
            {p.phase.replace("Phase ", "")}
          </span>
          <div className="rounded-2xl border border-line-soft bg-white p-6 sm:p-8">
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-xs font-semibold uppercase tracking-wide text-green-700">{p.phase}</span>
              <StatusBadge status={statusMap[p.status]} label={p.statusLabel} />
            </div>
            <h3 className="font-display mt-2 text-2xl font-semibold text-ink">{p.title}</h3>
            ...
            <p className="mt-5 rounded-xl bg-cream px-4 py-3 text-sm leading-relaxed text-ink-soft">{p.narrativ}</p>
          </div>
        </li>
      ))}
    </ol>
